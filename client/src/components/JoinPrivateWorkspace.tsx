...
const JoinPrivateWorkspace: React.FC<{
  workspaceId: string;
  isAlreadyRequested: boolean;
}> = ({ workspaceId, isAlreadyRequested }) => {
  return (
      ...
      <div className="max-w-md w-full space-y-8 text-center">
        ...
        <div className="space-y-3">
          ...
          <p className="text-brand-muted leading-relaxed font-medium">
            You've reached a private workspace. You must be a member or have an
            approved request to view the contents of{" "}
            <span className="text-brand-bright font-bold">Qubit</span>.
          </p>
        </div>
        ...
        <div className="bg-brand-card/50 border border-brand-border/20 p-6 rounded-2xl space-y-6">
          <div className="flex items-start gap-3 text-left p-3 bg-brand-surface/30 rounded-lg">
            ...
            <p className="text-xs text-brand-muted font-medium">
              Once you request access, the workspace owner will be notified to
              review your profile.
            </p>
          </div>

          <WorkspaceJoinRequest workspaceId={workspaceId!} already={isAlreadyRequested} />
        </div>
